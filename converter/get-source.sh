...
(cd $SRCDIR; git clone https://github.com/xiph/ogg.git; cd ogg; git checkout "$OGG_VER")
(cd $SRCDIR; git clone https://github.com/xiph/vorbis.git; cd vorbis; git checkout "$VORBIS_VER")
(cd $SRCDIR; git clone https://github.com/xiph/opus.git; cd opus; git checkout "$OPUS_VER")
...
(cd $SRCDIR; git clone https://anonscm.debian.org/git/pkg-gstreamer/orc.git/ orc; cd orc; git checkout "$ORC_VER")
