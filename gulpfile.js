...
function CopyBinary(platform, arch, extraPath = "") {
  let { binaryName } = PkgNames(platform, arch);
  return new Promise((resolve, _reject) => {
...
function MakeDebFiles(platform, arch) {
  let archName = { 64: "amd64", 32: "i386" }[arch];
  let appPath = "/opt/" + config.id;
  let control = Object.assign({}, config.deb, {
    package: config.id,
...
function MakeDeb(platform, arch, callback) {
  let archName = { 64: "amd64", 32: "i386" }[arch];
  let debName = config.id + "-" + manifest.version + "-1_" + archName + ".deb";
...
function MakeTarGzFiles(platform, arch) {
  let appPath = "/" + config.id + "-" + manifest.version;
  return Promise.all([
...
function MakeTarGz(platform, arch) {
  let archName = { 64: "amd64", 32: "i386" }[arch];
  let tarName = config.id + "-" + manifest.version + "-1_" + archName + ".tar";
...
function MakeIssFiles() {
  ...
  return Promise.all([
    ...
    new Promise((resolve, _reject) => {
      let basedir = path.relative("dist/win/iss", ".");
...
function MakeIss() {
  return new Promise((resolve, _reject) => {
    let issProcess = spawn(wineBinary, [issBinary, "dist/win/iss/app.iss"], {
      env: {
        WINEDEBUG: "-all"
      }
    });
...
gulp.task("check-wine", (_callback) => {
  return new Promise((resolve, reject) => {
...
gulp.task("check-iss", (_callback) => {
  ...
  function CheckNextPath() {
    let path = binaryCandidates.shift();
    if (!path) {
...
let osslsigncodePath = null;

function ExecSign(args) {
  return new Promise((resolve, _reject) => {
    let osslsigncodeProcess = spawn(osslsigncodePath, args);
    osslsigncodeProcess.stderr.on("data", (data) => {
      process.stderr.write(data);
    });
    osslsigncodeProcess.stdout.on("data", (_data) => {
      //process.stderr.write(data);
    });
...
function SignFiles(dir, _filter) {
  return new Promise((resolve, reject) => {
    let promises = [];
  ...
  switch (type) {
    case "dmg":
      ...
      promises.push(new Promise((resolve, reject) => {
        ...
        .then((lnPath) => {
          return new Promise((resolve, reject) => {
            let lnProcess = spawn(lnPath, ["-s", "-f", "/Applications", "dist/mac/dmg/Application"]);
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((path) => {
      return new Promise((resolve, _reject) => {
        ...
        let cmdProcess = spawn(path, args);
        ...
        cmdProcess.on("exit", (exitCode) => {
          let stderrStr = Buffer.concat(stderr).toString("utf8");
          if (exitCode) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      new Promise((resolve, reject) => {
        ...
        .then((npmPath) => {
          return new Promise((resolve, reject) => {
            let version = manifest.wehOptionalDevDependencies &&
              manifest.wehOptionalDevDependencies[packageName] || null;
            let pkgVersion = packageName + (version && "@" + version || "");
            console.info("Installing", pkgVersion);
            let npmProcess = spawn(npmPath, ["install", "--no-save", pkgVersion]);
...
gulp.task('unsetup-local-win', (callback) => {
  ...
  function DeleteKey(regPath, keyName) {
    return new Promise((resolve, _reject) => {
      regedit.deleteKey(regPath + "\\" + keyName, (_err) => {
        resolve();
      });
    });
  }
